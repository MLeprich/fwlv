"""
Magazine Staff Management Views
Personal-Verwaltung für Magazin-Modul
...
class MagazineStaffManagementView(LoginRequiredMixin, TemplateView):
    """
    Personal-Verwaltung für Magazine-Modul

    Modulverantwortliche können hier:
    - Benutzer als Bearbeiter (magazine_editors) zuweisen
    - Benutzer als Lesende (magazine_readers) zuweisen
    - Übersicht über alle zugewiesenen Personen erhalten
    """
    template_name = 'magazine/staff_management.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        editors_group, _ = Group.objects.get_or_create(name='magazine_editors')
        readers_group, _ = Group.objects.get_or_create(name='magazine_readers')
        ...
        context['is_module_admin'] = self.request.user.groups.filter(
            name__in=[Roles.ADMINISTRATOR, 'magazine_module_admin']
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (request.user.groups.filter(
            ...
            return redirect('magazine:staff_management')
        ...
        editors_group = Group.objects.get(name='magazine_editors')
        readers_group = Group.objects.get(name='magazine_readers')
        ...
        if action == 'assign':
            if role == 'editor':
                ...
                # Weise Magazine-Permissions zu
                self._assign_editor_permissions(user)

                messages.success(
                    request,
                    f'{user.get_full_name()} wurde als Bearbeiter für Magazin zugewiesen.'
                )

            elif role == 'reader':
                # Füge zu Readers hinzu (nur wenn nicht bereits Editor)
                if not user.groups.filter(name='magazine_editors').exists():
                    ...
                    messages.success(
                        request,
                        f'{user.get_full_name()} wurde als Lesender für Magazin zugewiesen.'
                    )
                else:
                    messages.warning(
                        request,
            ...
            # Entferne Magazine-Permissions
            self._remove_magazine_permissions(user)

            messages.success(
                request,
                f'{user.get_full_name()} wurde aus dem Magazin-Team entfernt.'
            )
    ...
    def _assign_editor_permissions(self, user):
        """Weise alle Magazine CRUD-Permissions zu"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

        # Alle Magazine-Models
        magazine_models = [
            'magazineitem',
    ...
    def _remove_magazine_permissions(self, user):
        """Entferne alle Magazine-Permissions"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

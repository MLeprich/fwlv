"""
Workshop Inventory Views

Views für die Inventur-Funktionalität im Workshop-Modul
"""
...
from .models import (
    ...
    WorkshopInventoryCheckItem,
    WorkshopToolInstance
)
from personnel.models import Person


class WorkshopInventoryListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    """Liste aller Workshop-Inventuren"""
    model = WorkshopInventoryCheck
    template_name = 'workshop/inventory/check_list.html'
    context_object_name = 'checks'
    permission_required = 'workshop.view_workshopinventorycheck'
    paginate_by = 20

    def get_queryset(self):
        queryset = WorkshopInventoryCheck.objects.select_related(
            'responsible_person',
            'location',
            'created_by'
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'module_name': 'KFZ-Werkstatt',
            'module_icon': '🔧',
            'module_color_from': 'from-orange-600',
            'module_color_to': 'to-red-600',
            'module_dashboard_url': 'workshop:dashboard',
            'inventory_list_url': 'workshop:inventory_list',
            'inventory_create_url': 'workshop:inventory_create',
            'inventory_detail_url': 'workshop:inventory_detail',
            'inventory_start_url': 'workshop:inventory_start',
            'inventory_count_url': 'workshop:inventory_count',
            'inventory_complete_url': 'workshop:inventory_complete',
            'current_module': 'workshop',

            # Stats
            'stats': {
                'in_progress': WorkshopInventoryCheck.objects.filter(status='in_progress').count(),
                'planned': WorkshopInventoryCheck.objects.filter(status='planned').count(),
                'completed_this_year': WorkshopInventoryCheck.objects.filter(
                    status='completed',
...
class WorkshopInventoryCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    """Neue Workshop-Inventur erstellen"""
    model = WorkshopInventoryCheck
    template_name = 'workshop/inventory/check_form.html'
    permission_required = 'workshop.add_workshopinventorycheck'
    fields = [
        'title',
        'description',
        'check_type',
    ...
    def get_success_url(self):
        return reverse('workshop:inventory_detail', kwargs={'pk': self.object.pk})
    ...
    def get_context_data(self, **kwargs):
        ...
        context['module_name'] = 'KFZ-Werkstatt'
        context['action'] = 'create'
        return context


class WorkshopInventoryDetailView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    """Detail-Ansicht einer Workshop-Inventur"""
    model = WorkshopInventoryCheck
    template_name = 'workshop/inventory/check_detail.html'
    context_object_name = 'check'
    permission_required = 'workshop.view_workshopinventorycheck'

    def get_queryset(self):
        return WorkshopInventoryCheck.objects.select_related(
            'responsible_person',
            'location',
            'approved_by',
            'created_by',
    ...
    def get_context_data(self, **kwargs):
        ...
        # Workshop-spezifisch
        if check.check_tool_condition:
            context['damaged_tools_total'] = check.items.filter(
                is_damaged=True
            ).count()

        if check.check_calibration:
            context['calibration_due_total'] = check.items.filter(
                calibration_due=True
            ).count()

        if check.check_safety_equipment:
            context['safety_issues_total'] = check.items.filter(has_safety_issue=True).count()
...
class WorkshopInventoryStartView(LoginRequiredMixin, PermissionRequiredMixin, View):
    """Startet eine geplante Inventur"""
    permission_required = 'workshop.change_workshopinventorycheck'

    def post(self, request, pk):
        check = get_object_or_404(WorkshopInventoryCheck, pk=pk)

        if not check.can_start():
            messages.error(request, 'Diese Inventur kann nicht gestartet werden.')
            return redirect('workshop:inventory_detail', pk=pk)

        # Starte Inventur
        if check.start_counting(request.user):
            # Generiere Inventur-Items aus aktuellem Workshop-Bestand
            items_created = 0

            # Erstelle Items aus WorkshopToolInstance (Werkzeuge)
            devices = WorkshopToolInstance.objects.filter(
                is_active=True,
                is_operational=True
            ...
            for device in devices:
                try:
                    ...
                    WorkshopInventoryCheckItem.objects.create(
                        inventory_check=check,
                        workshop_device=device,
                        location=device.location,
                        item_name=device.master.name if device.master else 'Werkzeug',
                    ...
                    logger.error(f'Fehler beim Erstellen von Inventur-Item für Workshop-Device {device.pk}: {str(e)}')
                    continue
            ...
            if items_created == 0:
                messages.warning(
                    ...
                    f'Bitte legen Sie zuerst Werkzeuge an.'
                )
            else:
                messages.success(
                    request,
...
class WorkshopInventoryCountingView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    """Zähl-Interface für Inventur"""
    model = WorkshopInventoryCheck
    template_name = 'workshop/inventory/counting_interface.html'
    context_object_name = 'check'
    permission_required = 'workshop.view_workshopinventorycheck'
    ...
    def get_context_data(self, **kwargs):
        ...
        items = items.select_related('location', 'workshop_device').order_by('location', 'item_name')
...
class WorkshopInventoryCompleteView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if check.counted_items < check.total_items:
            ...
            return redirect('workshop:inventory_count', pk=pk)
...
class WorkshopInventoryApproveView(LoginRequiredMixin, PermissionRequiredMixin, View):
    """Genehmigt und finalisiert eine Inventur"""
    permission_required = 'workshop.approve_workshopinventorycheck'
...
class WorkshopInventoryItemUpdateView(LoginRequiredMixin, PermissionRequiredMixin, View):
    """HTMX Endpoint: Aktualisiert einzelnes Inventur-Item"""
    permission_required = 'workshop.change_workshopinventorycheck'

    def post(self, request, pk):
        item = get_object_or_404(WorkshopInventoryCheckItem, pk=pk)

        # Prüfe ob Inventur noch in Bearbeitung
        if item.inventory_check.status != 'in_progress':
            return render(request, 'workshop/inventory/partials/item_row.html', {
                'item': item,
...
class WorkshopInventoryProgressView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(request, 'workshop/inventory/partials/progress_display.html', {
            'check': check
        })


class WorkshopInventoryExportView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def export_excel(self, check):
        ...
        writer.writerow([])
        writer.writerow(['Workshop-spezifische Prüfungen:'])
        writer.writerow(['Beschädigte Werkzeuge gefunden:', check.damaged_tools_found])

        if check.check_calibration:
            writer.writerow(['Kalibrierungen fällig:', check.calibration_due_found])

        if check.check_safety_equipment:
            writer.writerow(['Sicherheitsprobleme gefunden:', check.safety_issues_found])
    ...
    def export_pdf(self, check):
        ...
        try:
            ...
            html_string = render_to_string('workshop/inventory/export_pdf.html', {
                'check': check,
                'items': check.items.all().order_by('location__name', 'item_name'),
            ...
            return redirect('workshop:inventory_detail', pk=check.pk)

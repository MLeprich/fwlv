"""
Height Rescue Staff Management Views
Personal-Verwaltung für Höhenrettungs-Modul
...
class HeightRescueStaffManagementView(LoginRequiredMixin, TemplateView):
    """
    Personal-Verwaltung für Höhenrettungs-Modul

    Modulverantwortliche können hier:
    - Benutzer als Bearbeiter (height_rescue_editors) zuweisen
    - Benutzer als Lesende (height_rescue_readers) zuweisen
    - Übersicht über alle zugewiesenen Personen erhalten
    """
    template_name = 'height_rescue/staff_management.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        editors_group, _ = Group.objects.get_or_create(name='height_rescue_editors')
        readers_group, _ = Group.objects.get_or_create(name='height_rescue_readers')
        ...
        context['is_module_admin'] = self.request.user.groups.filter(
            name__in=[Roles.ADMINISTRATOR, 'height_rescue_module_admin']
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (request.user.groups.filter(
            ...
            return redirect('height_rescue:staff_management')
        ...
        editors_group = Group.objects.get(name='height_rescue_editors')
        readers_group = Group.objects.get(name='height_rescue_readers')
        ...
        if action == 'assign':
            if role == 'editor':
                ...
                # Weise Height Rescue-Permissions zu
                self._assign_editor_permissions(user)

                messages.success(
                    request,
                    f'{user.get_full_name()} wurde als Bearbeiter für Höhenrettung zugewiesen.'
                )

            elif role == 'reader':
                # Füge zu Readers hinzu (nur wenn nicht bereits Editor)
                if not user.groups.filter(name='height_rescue_editors').exists():
                    ...
                    messages.success(
                        request,
                        f'{user.get_full_name()} wurde als Lesender für Höhenrettung zugewiesen.'
                    )
                else:
                    messages.warning(
                        request,
            ...
            # Entferne Height Rescue-Permissions
            self._remove_height_rescue_permissions(user)

            messages.success(
                request,
                f'{user.get_full_name()} wurde aus dem Höhenrettungs-Team entfernt.'
            )
    ...
    def _assign_editor_permissions(self, user):
        """Weise alle Height Rescue CRUD-Permissions zu"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

        # Alle Height Rescue-Models
    ...
    def _remove_height_rescue_permissions(self, user):
        """Entferne alle Height Rescue-Permissions"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

"""
IT Hardware Staff Management Views
Personal-Verwaltung für IT-Hardware-Modul
...
class ITHardwareStaffManagementView(LoginRequiredMixin, TemplateView):
    """
    Personal-Verwaltung für IT-Hardware-Modul

    Modulverantwortliche können hier:
    - Benutzer als Bearbeiter (it_hardware_editors) zuweisen
    - Benutzer als Lesende (it_hardware_readers) zuweisen
    - Übersicht über alle zugewiesenen Personen erhalten
    """
    template_name = 'it_hardware/staff_management.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        editors_group, _ = Group.objects.get_or_create(name='it_hardware_editors')
        readers_group, _ = Group.objects.get_or_create(name='it_hardware_readers')
        ...
        context['is_module_admin'] = self.request.user.groups.filter(
            name__in=[Roles.ADMINISTRATOR, 'it_hardware_module_admin']
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (request.user.groups.filter(
            ...
            return redirect('it_hardware:staff_management')
        ...
        editors_group = Group.objects.get(name='it_hardware_editors')
        readers_group = Group.objects.get(name='it_hardware_readers')
        ...
        if action == 'assign':
            if role == 'editor':
                ...
                # Weise IT Hardware-Permissions zu
                self._assign_editor_permissions(user)

                messages.success(
                    request,
                    f'{user.get_full_name()} wurde als Bearbeiter für IT-Hardware zugewiesen.'
                )

            elif role == 'reader':
                # Füge zu Readers hinzu (nur wenn nicht bereits Editor)
                if not user.groups.filter(name='it_hardware_editors').exists():
                    ...
                    messages.success(
                        request,
                        f'{user.get_full_name()} wurde als Lesender für IT-Hardware zugewiesen.'
                    )
                else:
                    messages.warning(
                        request,
            ...
            # Entferne IT Hardware-Permissions
            self._remove_it_hardware_permissions(user)

            messages.success(
                request,
                f'{user.get_full_name()} wurde aus dem IT-Hardware-Team entfernt.'
            )
    ...
    def _assign_editor_permissions(self, user):
        """Weise alle IT Hardware CRUD-Permissions zu"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

        # Alle IT Hardware-Models
        it_hardware_models = [
    ...
    def _remove_it_hardware_permissions(self, user):
        """Entferne alle IT Hardware-Permissions"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission

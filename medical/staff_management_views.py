"""
Medical Staff Management Views
Personal-Verwaltung für Rettungsdienst-Modul
...
class MedicalStaffManagementView(LoginRequiredMixin, TemplateView):
    """
    Personal-Verwaltung für Medical-Modul

    Modulverantwortliche können hier:
    - Benutzer als Bearbeiter (medical_editors) zuweisen
    - Benutzer als Lesende (medical_readers) zuweisen
    - Übersicht über alle zugewiesenen Personen erhalten
    """
    template_name = 'medical/staff_management.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        editors_group, _ = Group.objects.get_or_create(name='medical_editors')
        readers_group, _ = Group.objects.get_or_create(name='medical_readers')
        ...
        context['is_module_admin'] = self.request.user.groups.filter(
            name__in=[Roles.ADMINISTRATOR, 'medical_module_admin']
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (request.user.groups.filter(
            ...
            return redirect('medical:staff_management')
        ...
        editors_group = Group.objects.get(name='medical_editors')
        readers_group = Group.objects.get(name='medical_readers')
        ...
        if action == 'assign':
            if role == 'editor':
                ...
                # Weise Medical-Permissions zu
                self._assign_editor_permissions(user)

                messages.success(
                    request,
                    f'{user.get_full_name()} wurde als Bearbeiter für Rettungsdienst zugewiesen.'
                )

            elif role == 'reader':
                # Füge zu Readers hinzu (nur wenn nicht bereits Editor)
                if not user.groups.filter(name='medical_editors').exists():
                    ...
                    messages.success(
                        request,
                        f'{user.get_full_name()} wurde als Lesender für Rettungsdienst zugewiesen.'
                    )
                else:
                    messages.warning(
                        request,
            ...
            # Entferne Medical-Permissions
            self._remove_medical_permissions(user)

            messages.success(
                request,
                f'{user.get_full_name()} wurde aus dem Rettungsdienst-Team entfernt.'
            )
    ...
    def _assign_editor_permissions(self, user):
        """Weise alle Medical CRUD-Permissions zu"""
    ...
    def _remove_medical_permissions(self, user):
        """Entferne alle Medical-Permissions"""
        from django.contrib.contenttypes.models import ContentType
        from django.contrib.auth.models import Permission
